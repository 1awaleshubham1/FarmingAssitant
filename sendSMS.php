



<html>
    <head>
        <meta charset="UTF-8">
        <title>Farming Assistant</title>
        <link href="mydesign.css" rel="stylesheet" type="text/css"/>
		<link rel="icon" href="images/sms.jfif" type="image/x-icon"/> 
    </head>

    <body>

        <div id="google_translate_element" ></div>

<script type="text/javascript">
function googleTranslateElementInit() {
  new google.translate.TranslateElement({pageLanguage: 'en', layout: google.translate.TranslateElement.InlineLayout.SIMPLE}, 'google_translate_element');
}
</script>

<script type="text/javascript" src="//translate.google.com/translate_a/element.js?cb=googleTranslateElementInit">
</script>
       
        <div id="header"></div>        
        
        <div id="menu">
            <ul>
       
      
                <li><a href="displaycomplaint.php">View Complaints</a></li>
				<li><a href="sendSMS.php">Send SMS </a></li>
        
            <li><a href="Logout.php">Logout</a></li>
          </ul>
        </div>
        
        <div id="content">
            <center>
            <iframe src="https://www.way2sms.com" width="1200" height="600"></iframe>
			</center>
        </div>
        <div id="footer">Copyright protected by M.G.M's COE Nanded</div>
        
    </body>


</html>
